...
junk <- table(wp.samp$A, wp.samp$C, wp.samp$E)
junk
junk2 <- make.gRbase.table_sb(c("A","C","E"), vals=junk, wp)

nds <- c("A","C")
make.gRbase.table_sb(nds, vals=wp.samp[nds], wp, normalize="first")
...
dim(junk2)
class(junk)                    # table
class(as.numeric(junk))        # numeric
class(wp.samp[c("A","C","E")]) #data.frame
