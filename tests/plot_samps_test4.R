...
wp.samp <- sample.crf(crf = wp, size = 1000, crf.sample.method = sample.junction, dress.sampleQ = T)
...
library(DescTools)
wp$edge.name.tab
epn <- round(100*wp$gR$edge.potentials[[1]]/sum(wp$gR$edge.potentials[[1]]))
eb  <- round(100*infer.junction(wp)$edge.bel[[1]])
CramerV(epn)
Lambda(epn)
CramerV(eb)
Lambda(eb)
