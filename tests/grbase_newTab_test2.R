...
# Define node state space and configuration space (weird Potts on purpose):
sA1 <- "A1"; sA2 <- "A2"
...
wp.samp <- sample.crf(crf = wp, size = 1000,
...
#nds <- c("D","E","B")
nds <- c("D")
tabNew.crf(nds, vals=wp.samp[nds], wp)
tabNew.crf(nds, vals=wp.samp[nds], wp, normalize="all")
tabNew.crf(nds, vals=wp.samp[nds], wp, normalize="first")
